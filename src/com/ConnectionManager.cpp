...
void ConnectionManager::createConnections(const std::unordered_map<std::string, std::string>& receiverAddresses,
...
void ConnectionManager::createConnection(const std::unordered_map<std::string, std::string>& connectionsList,
    ...
    for (auto& connection : connectionsList) {
        ...
        switch (connectionType) {
            case ConnectionManager::CONNECTION_TYPE::RECEIVER:
                m_receiverConnections.insert({name, std::make_unique<ReceiverConnection>(m_ioService, port)} );
            ...
            case ConnectionManager::CONNECTION_TYPE::SENDER:
                m_senderConnections.insert( {name, std::make_unique<SenderConnection>(m_ioService, port)} );
                Logger::getInstance().logInfo("Sender Connection created | " + name + ":" + std::to_string(port));
                break;
            default:
                Logger::getInstance().logError("CONNECTION DOESN'T HAS A TYPE | " + name + ":" + std::to_string(port));
        }
    }
}
